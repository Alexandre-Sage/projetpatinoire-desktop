...
class ChatFlow extends Component{
    constructor(props){
        ...
        const messageJsx= this.state.chatMessages.map((message, key)=>(
            <div key={key}>
                {message.userName===this.props.params.userName?<h3>Vous</h3>: <Link to={`/${this.props.params.userName}/userprofil/${message.userName}/${message.userId}`}>{message.userName}</Link>}
                <p>{message.content}</p>
                <p>{message.messageSendingDate}</p>
            </div>
        ));
